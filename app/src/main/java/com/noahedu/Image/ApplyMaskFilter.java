...
public class ApplyMaskFilter extends AbstractBufferedImageOp {
     ...
     */
	public ApplyMaskFilter( Bitmap maskImage, Bitmap destination ) {
     ...
     * @see #getDestination
     */
	public void setDestination( Bitmap destination ) {
		this.destination = destination;
	}
     ...
     */
	public static void composeThroughMask(Bitmap src, Bitmap dst, Bitmap sel) {
		int x = 0;//src.getMinX();
		int y = 0;//src.getMinY();
		int w = src.getWidth();
		int h = src.getHeight();
    ...
    public Bitmap filter( Bitmap src, Bitmap dst ) {
        ...
        if ( destination != null && maskImage != null )
			composeThroughMask( src, dst, maskImage);
